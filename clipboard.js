...
function renderTablesFromHTML(html) {
    ...
    tables.forEach((table, i) => {
        ...
        // CSV / TSV download buttons
        const btnRow = document.createElement("div");
        btnRow.className = "table-btn-row";
        ...
        dlCsv.href = csvUrl;
        dlCsv.download = tables.length > 1 ? `clipboard-table-${i + 1}.csv` : "clipboard-table.csv";
        dlCsv.className = "download-btn download-btn--csv";
        dlCsv.textContent = "Download CSV";
        btnRow.appendChild(dlCsv);

        const tsv = tableToTSV(liveTable);
        const tsvBlob = new Blob([tsv], { type: "text/tab-separated-values" });
        const tsvUrl = URL.createObjectURL(tsvBlob);
        const dlTsv = document.createElement("a");
        dlTsv.href = tsvUrl;
        dlTsv.download = tables.length > 1 ? `clipboard-table-${i + 1}.tsv` : "clipboard-table.tsv";
        dlTsv.className = "download-btn download-btn--csv";
        dlTsv.textContent = "Download TSV";
        btnRow.appendChild(dlTsv);
